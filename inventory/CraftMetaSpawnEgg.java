...
public class CraftMetaSpawnEgg extends CraftMetaItem implements SpawnEggMeta {
    ...
    void deserializeInternal(NBTTagCompound tag) {
        ...
        if (tag.hasKey(ENTITY_TAG.NBT)) {
            entityTag = tag.getCompoundTag(ENTITY_TAG.NBT);
            entityTag = (NBTTagCompound) MinecraftServer.getServer().dataFixer.update(FixTypes.ENTITY, new Dynamic(DynamicOpsNBT.a, entityTag), -1, CraftMagicNumbers.DATA_VERSION).getValue();
    ...
    final Material updateMaterial(Material material) {
        ...
        if (spawnedType != null) {
            if (entityTag != null) {
                ...
                entityTag.removeTag(ENTITY_ID.NBT);
            }
