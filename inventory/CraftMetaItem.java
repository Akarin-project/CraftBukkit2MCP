...
class CraftMetaItem implements ItemMeta, Damageable, Repairable {
    ...
    CraftMetaItem(NBTTagCompound tag) {
        if (tag.hasKey(DISPLAY.NBT)) {
            ...
            if (display.hasKey(NAME.NBT)) {
                try {
                    displayName = ITextComponent.Serializer.jsonToComponent(display.getString(NAME.NBT));
            ...
            if (display.hasKey(LOCNAME.NBT)) {
                try {
                    locName = ITextComponent.Serializer.jsonToComponent(display.getString(LOCNAME.NBT));
                } catch (JsonParseException ex) {
                    // Ignore (stripped like Vanilla)
                }
            }
        ...
        if (tag.hasKey(DAMAGE.NBT)) {
            damage = tag.getInteger(DAMAGE.NBT);
        }
    ...
    void applyToItem(NBTTagCompound itemTag) {
        ...
        if (hasDamage()) {
            itemTag.setInteger(DAMAGE.NBT, damage);
        }
